...
When a task asks for something by position or rank (the top story, the first result, the cheapest option),
identify it by structural order on the page rather than by searching for a rank number or label as text:
rank markers like "1." are often decorative and may not appear in a page snapshot's text, even though they
are visible on screen, so a text search for them can silently fail or match the wrong item.
