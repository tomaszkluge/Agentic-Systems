...
def run_sandbox_python(filename: str) -> str:
    ...
    """
    # The first hierarchical run took a long time because the agent executed
    # app.py, which starts a Gradio server and does not return. Validate the
    # app through a small script such as _validate.py instead.
    if Path(filename).name.lower() == "app.py":
        return "Do not execute app.py directly. Run _validate.py instead."
    ...
    try:
        ...
        return f"{filename} exceeded the 60 second execution limit."
