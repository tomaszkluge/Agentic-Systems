...
# this is not a tool - this is something we will run manually
def reset_sandbox() -> None:
    ...
    subprocess.run(
        ["uv", "init", "--bare", "--python", "3.13"], cwd=SANDBOX_DIR, check=True
    )
...
def run_sandbox_python(filename: str) -> str:
    ...
    result = subprocess.run(
        [
            ...
            "-w",
            "/workspace",
            "ghcr.io/astral-sh/uv:python3.13-bookworm-slim",  # we use this docker image, python 3.13 with uv already installed in there! (astral are the creators of uv)
            "uv",  # we are using uv to run the python file, instead of like in day 4 where it's `python solution.py`
            "run",
            filename,
        ],
        capture_output=True,
